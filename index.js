...
// نظام الرسائل (تسجيل دخول، خروج، غفوة، عودة) مع تقييد الأرماك بدقة
client.on('messageCreate', async message => {
    ...
    if (content === 'تسجيل دخول') {
        if (!settings.login) {
            return message.reply('❌ لم يتم تعيين روم تسجيل الدخول في هذا السيرفر بعد! اطلب من المسؤول استخدام أمر `/add-login`.');
        }
        if (channelId !== settings.login) {
            return message.reply(`❌ يرجى استخدام أمر "تسجيل دخول" في الروم المخصص له فقط: <#${settings.login}>`);
        }
    ...
    if (content === 'غفوة') {
        if (!settings.break) {
            return message.reply('❌ لم يتم تعيين روم البريك في هذا السيرفر بعد! اطلب من المسؤول استخدام أمر `/add-break`.');
        }
        if (channelId !== settings.break) {
            return message.reply(`❌ يرجى استخدام أمر "غفوة" في روم البريك المخصص فقط: <#${settings.break}>`);
        }
    ...
    if (content === 'عودة') {
        ...
        if (channelId !== settings.break) {
            return message.reply(`❌ يرجى استخدام أمر "عودة" في روم البريك المخصص فقط: <#${settings.break}>`);
        }
        const session = activeSessions.get(userId);
        if (!session) return message.reply('❌ أنت لم تسجل دخول.');
    ...
    if (content === 'تسجيل خروج') {
        if (!settings.logout) {
            return message.reply('❌ لم يتم تعيين روم تسجيل الخروج في هذا السيرفر بعد! اطلب من المسؤول استخدام أمر `/add-logout`.');
        }
        if (channelId !== settings.logout) {
            return message.reply(`❌ يرجى استخدام أمر "تسجيل خروج" في الروم المخصص له فقط: <#${settings.logout}>`);
        }
        const session = activeSessions.get(userId);
        if (!session) {
